    ...
    public class SharedMasterFormDetail_InputController : Controller
    {
        ...
        public JsonResult getBudgetByEO(string listEO, string companyId, string subjectId, string channelId, string brandId, string activityId, string status)
        {
            ...
            try
            {
                ...
                var groupEO = getListEO.Where(x => !string.IsNullOrEmpty(x.EO)).GroupBy(x => new { x.EO,x.IO}).Select((group, index) => new BudgetTotal
                {
                    EO = group.First().EO,
                    IO = group.First().IO,
                ...
                else
                {
                    ...
                    foreach (var item in groupEO)
                    {
                        ...
                        if (getAmount.Any())
                        {
                            ...
                            if (status == "2" || status == "3" || ConfigurationManager.AppSettings["Instock"].ToLower().Contains(item.IO.ToLower()) && item.IO != "")
                            {
                                item.total = 0;
                            }
                            else
