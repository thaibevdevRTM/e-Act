            ...
            try
            {
                ...
                var lists = (from DataRow d in ds.Tables[0].Rows
                             select new GetDataPVPrevious()
                             {
                                 ...
                                 totalallPayNo = objGetGetDataPVPrevious.master_type_form_id == ConfigurationManager.AppSettings["formPurchaseTbm"] ? GetBalancePV(d["activityId"].ToString()) : decimal.Parse(AppCode.checkNullorEmpty(d["totalallPayByIO"].ToString()) )
                             });
                return lists.ToList();
            }
            catch (Exception ex)
